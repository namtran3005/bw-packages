...
import { setIsInterestedInVisaCard } from '../setIsInterestedInVisaCard';
...
describe('setIsInterestedInVisaCard fn', () => {
  beforeAll(() => {
    jest
  ...
  it('should find one and update isInterestedInVisaCard with lean and return a promise', async () => {
    const res = await setIsInterestedInVisaCard('userId', true);
    expect(UserModel.findOneAndUpdate).toBeCalledWith(
      { _id: 'userId' },
      { $set: { isInterestedInVisaCard: true } },
      { runValidators: true }
    );
