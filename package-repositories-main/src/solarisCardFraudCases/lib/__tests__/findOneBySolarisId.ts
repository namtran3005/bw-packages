...
describe('findOneBySolarisId (find a card fraud case doc by solaris id)', () => {
  beforeAll(() => {
    jest
      .spyOn(SolarisCardFraudCaseModel, 'findOne')
  ...
  it('should find one with lean and return a promise', async () => {
    ...
    expect(SolarisCardFraudCaseModel.findOne).toBeCalledWith({
      solarisId: 'solarisId',
    });
