...
export interface MainSolarisYellowStatusExistingUsersVariables {
  firstName: string;
}

export const mainSolarisYellowStatusExistingUsers = (
  locale: Locales
): string => {
  const content = (key: string) =>
    intl.formatMessage(
      { id: `emails.mainSolarisYellowStatusExistingUsers.${key}` },
      locale
    );
  ...
  return `<mj-image mj-class="top-icon" src="${
    ...
    <mj-text mj-class="left">${content('intro')}</mj-text>
