import * as React from 'react';
import styled from 'styled-components';
import { Paper as MuiPaper } from '@material-ui/core';
import { PaperProps as MuiPaperProps } from '@material-ui/core/Paper';
import {
  Colors,
  ColorsType,
  Spacing,
  Transitions,
} from '@bitwala-cryptobank-squad/package-theme';
import { mediaStyledProperties } from '../../utils/mediaStyledProperty';
import {
  RootSpacingProps as ContentSpacingProps,
  MaxWidthType,
  MinWidthType,
} from '../Content/Content.styled';

const shadowOpacityMultiplier = 0.2;

const shadowKeyUmbraOpacity = 0.2 * shadowOpacityMultiplier;
const shadowKeyPenumbraOpacity = 0.14 * shadowOpacityMultiplier;
const shadowAmbientShadowOpacity = 0.12 * shadowOpacityMultiplier;

const createShadow = (...px: number[]) => {
  return [
    `${px[0]}px ${px[1]}px ${px[2]}px ${px[3]}px rgba(0, 0, 0, ${shadowKeyUmbraOpacity})`,
    `${px[4]}px ${px[5]}px ${px[6]}px ${px[7]}px rgba(0, 0, 0, ${shadowKeyPenumbraOpacity})`,
    `${px[8]}px ${px[9]}px ${px[10]}px ${px[11]}px rgba(0, 0, 0, ${shadowAmbientShadowOpacity})`,
  ].join(',');
};

const shadows = [
  'none',
  createShadow(0, 1, 3, 0, 0, 1, 1, 0, 0, 2, 1, -1),
  createShadow(0, 1, 5, 0, 0, 2, 2, 0, 0, 3, 1, -2),
  createShadow(0, 1, 8, 0, 0, 3, 4, 0, 0, 3, 3, -2),
  createShadow(0, 2, 4, -1, 0, 4, 5, 0, 0, 1, 10, 0),
  createShadow(0, 3, 5, -1, 0, 5, 8, 0, 0, 1, 14, 0),
  createShadow(0, 3, 5, -1, 0, 6, 10, 0, 0, 1, 18, 0),
  createShadow(0, 4, 5, -2, 0, 7, 10, 1, 0, 2, 16, 1),
  createShadow(0, 5, 5, -3, 0, 8, 10, 1, 0, 3, 14, 2),
  createShadow(0, 5, 6, -3, 0, 9, 12, 1, 0, 3, 16, 2),
  createShadow(0, 6, 6, -3, 0, 10, 14, 1, 0, 4, 18, 3),
  createShadow(0, 6, 7, -4, 0, 11, 15, 1, 0, 4, 20, 3),
  createShadow(0, 7, 8, -4, 0, 12, 17, 2, 0, 5, 22, 4),
  createShadow(0, 7, 8, -4, 0, 13, 19, 2, 0, 5, 24, 4),
  createShadow(0, 7, 9, -4, 0, 14, 21, 2, 0, 5, 26, 4),
  createShadow(0, 8, 9, -5, 0, 15, 22, 2, 0, 6, 28, 5),
  createShadow(0, 8, 10, -5, 0, 16, 24, 2, 0, 6, 30, 5),
  createShadow(0, 8, 11, -5, 0, 17, 26, 2, 0, 6, 32, 5),
  createShadow(0, 9, 11, -5, 0, 18, 28, 2, 0, 7, 34, 6),
  createShadow(0, 9, 12, -6, 0, 19, 29, 2, 0, 7, 36, 6),
  createShadow(0, 10, 13, -6, 0, 20, 31, 3, 0, 8, 38, 7),
  createShadow(0, 10, 13, -6, 0, 21, 33, 3, 0, 8, 40, 7),
  createShadow(0, 10, 14, -6, 0, 22, 35, 3, 0, 8, 42, 7),
  createShadow(0, 11, 14, -7, 0, 23, 36, 3, 0, 9, 44, 8),
  createShadow(0, 11, 15, -7, 0, 24, 38, 3, 0, 9, 46, 8),
];

export default shadows;

type BasePaperProps = Omit<MuiPaperProps, 'ref'>;

interface RootProps extends BasePaperProps, ContentSpacingProps {
  background?: ColorsType;
  backgroundAlpha?: number;
  border?: boolean;
  elevation?: 0 | 1 | 2 | 4 | 6 | 8 | 9 | 12 | 16 | 24;
  highlight?: ColorsType;
  rounded?: boolean;
  maxWidth?: MaxWidthType;
  minWidth?: MinWidthType;
  zIndex?: React.CSSProperties['zIndex'];
  overflow?: React.CSSProperties['overflow'];
  innerRef?: any; // eslint-disable-line @typescript-eslint/no-explicit-any
}

const getPxValue = (value: number) => `${value}px`;

const RootComponent: React.FunctionComponent<RootProps> = ({
  /* eslint-disable @typescript-eslint/no-unused-vars */
  background,
  backgroundAlpha,
  border,
  spacing,
  horizontalSpacing,
  verticalSpacing,
  topSpacing,
  bottomSpacing,
  leftSpacing,
  rightSpacing,
  highlight,
  rounded,
  maxWidth,
  minWidth,
  overflow,
  zIndex,
  /* eslint-enable @typescript-eslint/no-unused-vars */
  ...cleanProps
}) => <MuiPaper elevation={1} {...cleanProps} />;

const defaultBackground = 'white';

const getPaperBackground = (props: RootProps) => {
  if (props.background) {
    if (props.backgroundAlpha !== undefined) {
      return Colors.getWithAlpha(props.background, props.backgroundAlpha);
    } else {
      return Colors.get(props.background);
    }
  }
  return Colors.get(defaultBackground);
};

const RootContent = styled.div``;

const Root = styled(RootComponent)`
  &&& {
    background: ${getPaperBackground};
    border-radius: 0;
    ${({ border }) =>
      border ? `border: 1px solid ${Colors.getWithAlpha('text', 0.05)}` : ''};
    ${({ highlight }) =>
      highlight
        ? `border-top: ${Spacing.unit / 2}px solid ${Colors.get(highlight)}`
        : ''};
    border-radius: ${({ rounded }) => (rounded ? `${Spacing.unit}px` : 'none')};
    box-shadow: ${({ elevation }) =>
      shadows[elevation === undefined ? 2 : elevation]};
    transition: ${Transitions.duration.normal}ms ${Transitions.easing.swiftOut};
    max-width: ${({ maxWidth }) => (maxWidth ? `${maxWidth}px` : 'none')};
    min-width: ${({ minWidth }) => (minWidth ? `${minWidth}px` : 'none')};
    box-sizing: border-box;
    overflow: ${({ overflow }) => (overflow ? overflow : 'initial')};
    z-index: ${({ zIndex }) => (zIndex ? zIndex : 'initial')};

    > ${RootContent} {
      box-sizing: border-box;
      transition: ${Transitions.duration.normal}ms
        ${Transitions.easing.swiftOut};

      ${({
        topSpacing,
        bottomSpacing,
        leftSpacing,
        rightSpacing,
        spacing = 24,
      }) =>
        mediaStyledProperties({
          'padding-top': {
            value: typeof topSpacing !== 'undefined' ? topSpacing : spacing,
            transform: getPxValue,
          },
          'padding-bottom': {
            value:
              typeof bottomSpacing !== 'undefined' ? bottomSpacing : spacing,
            transform: getPxValue,
          },
          'padding-left': {
            value: typeof leftSpacing !== 'undefined' ? leftSpacing : spacing,
            transform: getPxValue,
          },
          'padding-right': {
            value: typeof rightSpacing !== 'undefined' ? rightSpacing : spacing,
            transform: getPxValue,
          },
        })};
    }
  }
`;

Root.displayName = 'Paper';
RootComponent.displayName = 'Paper';

export { Root, RootProps, RootComponent, RootContent, shadows };
